...
import org.primefaces.diamond.dao.UsuarioRolDAO;
import org.primefaces.diamond.entity.UsuarioRol;
...
@Named
@ApplicationScoped
public class UsuarioRolService implements IService<UsuarioRol, Integer> {

    private static final Logger LOGGER = Logger.getLogger(UsuarioRolService.class);

    @Inject
    private UsuarioRolDAO dao;

    @Override
    public boolean create(UsuarioRol entity) {
        LOGGER.info(MessageFormat.format("Operacion de creacion de {0}", entity.getId()));
        return this.dao.create(entity);
    }

    @Override
    public boolean edit(UsuarioRol entity) {
        LOGGER.info(MessageFormat.format("Operacion de edicion de {0}", entity.getId()));
        return this.dao.edit(entity);
    }

    @Override
    public boolean remove(UsuarioRol entity) {
